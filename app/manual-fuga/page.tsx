...
export default function ManualFugaPage() {
  ...
  const documentos = [
    ...
    "Carteira de trabalho",
    "Comprovante de renda",
      // prefer sharing a generated PDF file if possible
      ;(async () => {
        try {
          const blob = await generatePdfBlob()
          if (!blob) throw new Error("Não foi possível gerar o PDF")
    ...
    const generatePdfBlob = async (): Promise<Blob | null> => {
      try {
        if (!printableRef.current) return null
        // dynamic imports so these libs are only loaded in client when needed
        const html2canvas = (await import('html2canvas')).default
        const { jsPDF } = await import('jspdf')
